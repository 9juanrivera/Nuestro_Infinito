...
const PATH_CARDS = [
  ...
  {
    date: "la más cercana / the closest one / a mais perto",
    title: {
      es: "Próxima Centauri",
      en: "Proxima Centauri",
      pt: "Próxima Centauri"
    ...
    text: {
      es: "Escribe aquí tu carta sobre Próxima Centauri. [Borrador: es la estrella más cercana a nosotros después del sol, así que la elegí para hablar de ti, de lo cerca que te siento incluso en los días en que la distancia se pone difícil. El universo es enorme, casi imposible de medir, y aun así hay una luz que decidió quedarse a nuestro lado, la más próxima de todas. Así te siento a ti: por más lejos que a veces estemos, siempre eres lo más cercano que tengo.]",
      en: "Write your letter about Proxima Centauri here. [Draft: it's the closest star to us after the sun, so I picked it to talk about you, about how close I feel you even on the days when distance gets hard. The universe is huge, almost impossible to measure, and still there's one light that chose to stay nearest to us. That's how I feel about you: no matter how far apart we sometimes are, you're always the closest thing I have.]",
      pt: "Escreva aqui a sua carta sobre Próxima Centauri. [Rascunho: é a estrela mais perto de nós depois do sol, por isso escolhi ela pra falar de você, de como eu te sinto perto mesmo nos dias em que a distância pesa. O universo é enorme, quase impossível de medir, e mesmo assim existe uma luz que decidiu ficar mais perto de nós que qualquer outra. É assim que eu sinto você: por mais longe que a gente às vezes esteja, você é sempre o que eu tenho de mais perto.]"
    }
  },
  {
    date: "un lugar por descubrir / a place still to discover / um lugar por descobrir",
    title: {
      es: "Kepler-1111",
      en: "Kepler-1111",
      pt: "Kepler-1111"
    ...
    text: {
      es: "Escribe aquí tu carta sobre Kepler-1111. [Borrador: es un planeta lejano, de esos que todavía estamos aprendiendo a conocer, girando alrededor de una estrella que ni siquiera podemos ver a simple vista. Me gusta pensar que se parece a lo que estamos construyendo: un lugar que no existía antes de nosotros, que nadie más ha visitado, que todavía tiene mucho por descubrirse. No sabemos exactamente cómo es, pero sabemos que ahí, en algún punto del futuro, hay una vida entera esperándonos, y quiero recorrerla toda contigo.]",
      en: "Write your letter about Kepler-1111 here. [Draft: it's a distant planet, one of those we're still learning about, orbiting a star we can't even see with the naked eye. I like to think it looks like what we're building: a place that didn't exist before us, that no one else has visited, that still has so much left to discover. We don't know exactly what it looks like yet, but we know that somewhere out there, in the future, a whole life is waiting for us, and I want to explore all of it with you.]",
      pt: "Escreva aqui a sua carta sobre Kepler-1111. [Rascunho: é um planeta distante, daqueles que a gente ainda tá aprendendo a conhecer, girando ao redor de uma estrela que nem dá pra ver a olho nu. Gosto de pensar que ele parece com o que a gente está construindo: um lugar que não existia antes de nós, que ninguém mais visitou, que ainda tem muito por descobrir. A gente não sabe exatamente como é, mas sabe que em algum ponto do futuro tem uma vida inteira esperando por nós, e eu quero viver ela toda com você.]"
    }
  }
];

/* =========================================================
   UNIVERSO — cuerpos celestes del mapa "Universo"

   Para agregar un cuerpo nuevo, copia uno de los bloques de
   abajo y cambia:
   - id:     un nombre único, en minúsculas.
   - kind:   "star", "planet", "comet" o "nebula" (solo referencia,
             el color y el tamaño son lo que realmente cambia el look).
   - x, y:   dónde aparece dentro del mapa, en % (0 a 100).
   - size:   qué tan grande se ve el punto, en píxeles.
   - color:  el color del punto y de su brillo (código hex).
   - tag:    una frase corta que aparece arriba del nombre, en la tarjeta.
   - name:   el nombre que se muestra bonito debajo del punto en el mapa.
   - luego, para el texto de la tarjeta, tienes dos opciones:
       a) pathIndex: si ya escribiste su carta en PATH_CARDS (arriba),
          pon el mismo título ahí abajo entre comillas para enlazarla.
       b) text: { es:"...", en:"...", pt:"..." } con una historia propia,
          si no quieres repetir una carta que ya existe en "Camino".
   ========================================================= */
function findPathIndexByTitle(esTitle) {
  return PATH_CARDS.findIndex(c => c.title.es === esTitle);
}

const UNIVERSE_BODIES = [
  {
    id: "proxima",
    kind: "star",
    x: 76, y: 22,
    size: 15,
    color: "#f2836b",
    pathIndex: findPathIndexByTitle("Próxima Centauri"),
    tag: { es: "la más cercana", en: "the closest one", pt: "a mais perto" },
    name: { es: "Próxima Centauri", en: "Proxima Centauri", pt: "Próxima Centauri" }
  },
  {
    id: "kepler",
    kind: "planet",
    ...
    size: 22,
    color: "#b9a6f2",
    pathIndex: findPathIndexByTitle("Kepler-1111"),
    tag: { es: "un lugar por descubrir", en: "a place still to discover", pt: "um lugar por descobrir" },
    name: { es: "Kepler-1111", en: "Kepler-1111", pt: "Kepler-1111" }
  }

  // Agrega aquí el próximo cuerpo cuando quieras sumar uno nuevo. Por ejemplo:
  // {
  //   id: "nuevo-cuerpo",
  //   kind: "star",
  //   x: 50, y: 15,
...
const I18N = {
  en: {
    ...
    "nav.camino": "Path",
    "nav.universo": "Universe",
    "universe.eyebrow": "our map",
    "universe.title": "The universe we're building",
    "universe.sub": "Tap each point to get to know it. Every new body that shows up here will get its own pretty name and its own story.",
    "universe.home": "Us",
    "universe.homeTag": "where it all starts",
  ...
  pt: {
    ...
    "nav.universo": "Universo",
    "universe.eyebrow": "nosso mapa",
    "universe.title": "O universo que estamos construindo",
    "universe.sub": "Toque em cada ponto para conhecê-lo. Cada corpo novo que aparecer aqui vai ganhar seu próprio nome bonito e sua própria história.",
    "universe.home": "Nós",
    "universe.homeTag": "onde tudo começa",
  ...
  es: {
    ...
    "nav.camino": "Camino",
    "nav.universo": "Universo",
    "universe.eyebrow": "nuestro mapa",
    "universe.title": "El universo que estamos armando",
    "universe.sub": "Toca cada punto para conocerlo. Cada cuerpo nuevo que aparezca aquí va a tener su propio nombre bonito y su propia historia.",
    "universe.home": "Nosotros",
    "universe.homeTag": "donde empieza todo",
...
function openCard(
  ...
  const eyebrow =
    `${t("modal.stop")
      .replace("{n}", index + 1)
      .replace("{total}", PATH_CARDS.length)
    } · ${card.date}`;

  showCardModal(card, eyebrow, btnEl);
}

// Abre la misma tarjeta modal, pero para un cuerpo del mapa "Universo".
// Si el cuerpo tiene pathIndex, reutiliza la carta ya escrita en PATH_CARDS;
// si no, usa su propio "text" (o el nombre como texto, si no escribiste nada aún).
function openUniverseNode(body, btnEl) {
  let card;

  if (
    typeof body.pathIndex === "number" &&
    body.pathIndex >= 0 &&
    PATH_CARDS[body.pathIndex]
  ) {
    card = PATH_CARDS[body.pathIndex];
  } else {
    card = {
      title: body.name,
      text: body.text || body.name
    };
  }

  const eyebrow =
    (body.tag && (body.tag[currentLang] || body.tag.es)) || "";

  showCardModal(card, eyebrow, btnEl);
}

function showCardModal(card, eyebrowText, btnEl) {
  currentCardBtn = btnEl || null;
...
// Dibuja el mapa del "Universo": el punto central (nosotros) y una
// línea punteada hacia cada cuerpo definido en UNIVERSE_BODIES.
function buildUniverse() {
  const nodesWrap = document.getElementById("universeNodes");
  const linesWrap = document.getElementById("universeLines");

  if (!nodesWrap || !linesWrap) return;
  ...
  let nodesHtml = `
    <div class="uni-node uni-node-home" style="left:${homeX}%; top:${homeY}%;">
      <span class="uni-node-dot"></span>
      <span class="uni-node-label">
        <span class="uni-node-tag">${t("universe.homeTag")}</span>
        <span class="uni-node-name">${t("universe.home")}</span>
      </span>
    </div>
  `;
  ...
  UNIVERSE_BODIES.forEach((body, i) => {
    const x = body.x;
    const y = body.y;
    const size = body.size || 16;
    const color = body.color || "#f6ece0";
    const name = body.name[currentLang] || body.name.es;
    const tag = (body.tag && (body.tag[currentLang] || body.tag.es)) || "";
      ...
      >
        <span
          ...
          style="
            ...
            background: radial-gradient(circle at 35% 30%, #fff, ${color} 55%, rgba(0,0,0,.55) 100%);
            box-shadow: 0 0 ${Math.round(size * 0.8)}px ${Math.round(size * 0.25)}px ${color}99,
                        0 0 ${Math.round(size * 2)}px ${Math.round(size * 0.6)}px ${color}33;
            color:${color};
          "
        ></span>
        <span class="uni-node-label">
          <span class="uni-node-tag">${tag}</span>
          <span class="uni-node-name">${name}</span>
        </span>
      </button>
    `;

    const x1 = (homeX / 100) * VB_W;
    const y1 = (homeY / 100) * VB_H;
    const x2 = (x / 100) * VB_W;
    const y2 = (y / 100) * VB_H;

    linesHtml += `<path class="uni-line" d="M ${x1} ${y1} L ${x2} ${y2}" />`;
  });
  ...
  nodesWrap
    .querySelectorAll(".uni-node[data-uni-index]")
    .forEach(btn => {
      btn.addEventListener("click", () => {
        const body =
          UNIVERSE_BODIES[Number(btn.dataset.uniIndex)];

        openUniverseNode(body, btn);
      });
    });
}

// Genera muchas mini-estrellas flotantes con posiciones y ritmos al azar,
// para que el fondo se sienta más denso y vivo (más "espacio" de verdad).
function initMiniStars() {
  const wrap = document.getElementById("miniStars");
  if (!wrap) return;

  const isSmall = window.innerWidth < 640;
  const count = isSmall ? 26 : 46;
  const colors = ["#f6ece0", "#d9b25e", "#f0a3b4"];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const dur = (Math.random() * 45 + 28).toFixed(1);
    const twinkleDur = (Math.random() * 4 + 2).toFixed(1);
    const delay = (-Math.random() * 40).toFixed(1);
    const color = colors[Math.floor(Math.random() * colors.length)];

    html += `<span class="mini-star" style="
      top:${top}%; left:${left}%;
      width:${size}px; height:${size}px;
      color:${color};
      animation-duration:${twinkleDur}s, ${dur}s;
...
// Movimiento sutil de las capas de fondo según la posición del cursor
// (o del dedo, en pantallas táctiles), para que el cielo se sienta interactivo.
function initParallax() {
  ...
  function apply() {
    far.style.transform = `translate(${targetX * -14}px, ${targetY * -14}px)`;
    mid.style.transform = `translate(${targetX * -26}px, ${targetY * -26}px)`;
    near.style.transform = `translate(${targetX * -42}px, ${targetY * -42}px)`;
    raf = null;
  }
...
function initStarfield() {
  ...
  // ---- Estrellas fugaces mágicas ----
  let shootingStars = [];
  let nextShootTime = 0;

  function maybeSpawnShootingStar(t) {
    if (t < nextShootTime) return;

    nextShootTime =
      t + 3500 + Math.random() * 6000;
  ...
  function drawShootingStars() {
    shootingStars.forEach(sh => {
      ...
      const grad = ctx.createLinearGradient(
        sh.x, sh.y, tailX, tailY
      );
      grad.addColorStop(0, `rgba(246,236,224,${sh.life})`);
      grad.addColorStop(1, "rgba(246,236,224,0)");
  ...
  function drawSparkle(st, alpha, color) {
    const len = st.r * 5 + 2;
  ...
  function draw(t) {
    ...
    stars.forEach(st => {
      ...
      if (st.sparkle && !reduceMotion) {
        drawSparkle(st, ctx.globalAlpha, st.color);
      }
    });
